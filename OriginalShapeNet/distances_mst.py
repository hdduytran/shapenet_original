...
def dis4(query_topk,proto,train_labels,n=3):
    ...
    # proto = np.reshape(proto,(num_classes*query_topk.shape[1],query_topk.shape[-1]))
    # proto_labels = np.array(np.unique(train_labels).repeat(query_topk.shape[1]))
    # print('protoo',proto.shape)
    # print(proto_labels)

    # mean_query = query_topk.squeeze().mean(axis=1)
    # print(mean_query.shape)
    knn.fit(proto,train_labels)

    prediction = knn.predict(query_topk)
    # print(prediction)
    return prediction
...
def dis5(query_topk,proto,train_labels,n=3):
    ...
    prediction = []
    for i in query_topk:
        pred = knn.predict(i.reshape(1, -1))
