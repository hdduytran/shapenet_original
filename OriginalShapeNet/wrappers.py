...
class TimeSeriesEncoderClassifier(sklearn.base.BaseEstimator,
    ...
    def new_shapelet_discovery(self, X, train_labels, cluster_num, batch_size = 50):
        ...
        for i in range(num_cluster):
            candidate_cluster_size.append(representation_all[representation_class_label==i][:,0].size) # số lượng shapelet trong cụm
            dim_in_cluster_i = list()
            class_label_cluster_i = list()
            dist = math.inf
            for j in range(representation_all[representation_class_label==i][:,0].size): # duyệt qua từng feature map
